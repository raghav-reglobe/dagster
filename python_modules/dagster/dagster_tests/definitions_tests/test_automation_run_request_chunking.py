...
def test_chunked_entity_keys_for_runs() -> None:
    ...
    # no limit -> single chunk containing everything
    assert _chunked_entity_keys_for_runs(keys, None) == [keys]

    # at or under the limit -> single chunk
    assert len(_chunked_entity_keys_for_runs(keys, 10)) == 1

    chunks = _chunked_entity_keys_for_runs(keys, 4)
    assert [len(chunk) for chunk in chunks] == [4, 4, 2]
    # deterministic ordering, disjoint, complete
    assert [key for chunk in chunks for key in chunk] == sorted(
        keys, key=lambda key: key.to_user_string()
    )
...
def test_build_run_requests_max_entities_per_run() -> None:
    ...
    all_keys = [
        key for run_request in run_requests for key in (run_request.asset_selection or [])
    ]
